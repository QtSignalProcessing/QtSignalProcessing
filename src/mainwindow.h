...
Q_OBJECT
    ...
    public:
        ...
        QList<float> pointer2Qvec(float* data,int size);
...
signals:
    ...
    private:
        ...
        struct SamplePara
        {
            ...
            QList<float> sample_data;
        };

    PlotWidget* _SampledWave;
    QList<float> _audio_data;
    plotFilter* _FilterWidget;
    PlotWidget *_DisSpec;
    PlotWidget *_OrgWave;
    PlotWidget *_ConSpec;
    QSpinBox *bitBox;
    QList<float> _quantized_data;
    int bits;
    void createActions();
    void createMenus();
    void filter();
